...
模型路由已作为本项目默认策略，不需要每次点名技能或模型。跨多个阶段时，AI应自动调用统一路由入口；下面的显式写法只用于外部AI未读取项目 `AGENTS.md` 的情况：

```text
请使用 vinno-model-routing 处理这批材料。按阶段在 Luna、Terra、Sol 中选择最低充分模型；注册红线冲突和正式发布必须升级审核并保留我的确认。
```

路由技能只决定模型、业务技能和升级路径，不替代下面五个业务技能。用户明确指定模型时以当次要求为准。执行环境不支持按任务选择模型时，它会明确报告限制，而不会假装已经切换。
